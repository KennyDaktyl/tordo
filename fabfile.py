...
env.export = f'export DB_USER={os.environ.get("DB_USER")} && export DB_PASSWORD={os.environ.get("DB_PASSWORD")}'.format(env.directory)
...
def deploy():
    ...
    with virtualenv():
        run('sudo git pull')
        run('sudo /home/kenny/www/tordo/env/bin/python manage.py collectstatic --noinput')
...
def backup_db():
    ...
    run('sudo PGPASSWORD="' + password +
          '" pg_dump -U postgres -h localhost tordo_dev > /home/kenny/www/tordo/media/files/tordo_dev_backup.sql')
    local(f'sshpass -p {os.environ.get("SSH_PASSWORD")} scp -r -P 62211 kenny@51.75.64.242:/home/kenny/www/tordo/media/files/tordo_dev_backup.sql /home/{local_user}/Pulpit/Backup')
    local(f'PGPASSWORD="{os.environ.get("SSH_PASSWORD")} dropdb -U postgres -h localhost tordo_dev')
    local(f'PGPASSWORD="{os.environ.get("SSH_PASSWORD")} createdb -U postgres -h localhost tordo_dev')
    local(f'PGPASSWORD="Tofik123!" psql -U postgres -h localhost tordo_dev < /home/{local_user}/Pulpit/tordo_dev_backup.sql')
